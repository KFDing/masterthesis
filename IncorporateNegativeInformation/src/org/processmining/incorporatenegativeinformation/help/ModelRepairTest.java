...
/**
 * this class is used to test the reason fot interface is found but class is
 * expected.. to find out the bug location, in jar file or the setting in
 * KNIME??
 * 
 * @author dkf
 *
...
@Plugin(name = "Test jar for repair model", level = PluginLevel.Regular, returnLabels = { "Net", "Initial Marking",
		"Final Marking" }, returnTypes = { Petrinet.class, Marking.class,
				Marking.class }, parameterLabels = { "Log", "Accepting Petri net" }, userAccessible = true)
...
public class ModelRepairTest {
	...
	public Object[] repair(PluginContext context, XLog log, AcceptingPetriNet anet) {
		...
		Object[] result = repairer.repairModel_buildT2Econnection(context, log, anet.getNet(), anet.getInitialMarking(),
				anet.getFinalMarkings().iterator().next(), config, classifier);
