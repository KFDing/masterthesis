...
/**
 * this class is used to delete silent transition after the model generation
 * in order to make model better looking..
 * @author ding
 *
 */

@Plugin(name = "Delete Silent Transition in Seq", level = PluginLevel.Regular, returnLabels = {"Reduced Net" }, returnTypes = {
		Petrinet.class}, parameterLabels = { "Petri net"}, userAccessible = true)
public class SilentTransitionDeletor {
	...
	/**
	 * this method is based the limits for one transition transition: 
	 *   only in sequence situation, we can deal with easier. 
	 *   p1-->t-->t2
	 *  if |Outedges(p1)| >= 2 and |Inedges(p2)| >=2, 
	 *     this silent transition can not deleted..
	 *  else,
	 *     we can delete the silent transition,
	 * 
	...
	public static Petrinet deleteForSeq(Petrinet onet) {
		...
		for(Transition t : ts) {
			if(t.isInvisible()){
				...
				if(tInEdges.size()>1)
					continue;
				Collection<PetrinetEdge<? extends PetrinetNode, ? extends PetrinetNode>> tOutEdges = onet.getOutEdges(t);
				if(tOutEdges.size()>1)
					continue;
				...
				Collection<PetrinetEdge<? extends PetrinetNode, ? extends PetrinetNode>> pOutEdges = onet.getOutEdges(source);
				Collection<PetrinetEdge<? extends PetrinetNode, ? extends PetrinetNode>> pInEdges = onet.getInEdges(target);
				if(pOutEdges.size()>1 && pInEdges.size() > 1) {
					//keep this silent transition
					continue;
				}else {
					// delete this silent transition
					deleteSilentTransition(dnet, (Transition)map.get(t), (Place)map.get(source), (Place)map.get(target));
				}
	...
	/**
	 * here we delete one silent transition tau from net
	 *  steps are :
	 *      find the places before and after the tau
	 *      count the outedges and inedges for places
	 *      keep the place with edges number > 1, 
	 *      delete the place with edges number < 2
	 *  to keep it simple, we can give more information: the place to delete
	 *    :: but still the place before, because we need to pass the edges 
	 *    to them    
	 * @param net
	 * @param tau
	 * @param source
	...
	public static Petrinet deleteSilentTransition(Petrinet net, Transition tau, Place source, Place target) {
		...
		if(pOutEdges.size()<2) {
			...
			Collection<PetrinetEdge<? extends PetrinetNode, ? extends PetrinetNode>> tOutEdges = net.getOutEdges(target);
			// connect all the sInEdges to place target
			for(PetrinetEdge outEdge: tOutEdges) {
				// if we want to change the sInEdges connection, we need to get all the upstream transitions
				// can not set the inEdge target directly..
